    ...
    <a-layout-content id="forgot-layout">
      ...
      >
        ...
        >
          ...
          <!-- 重置验证码 -->
          <a-form-item label="Reset code">
            <a-input
              style="float: left; width: 55%"
              v-decorator="[
                'captcha',
                {
                  rules: [{ required: true, message: 'Please enter the reset verification code' }],
                },
              ]"
              placeholder="Please enter the reset verification code"
            >
              <a-icon
                slot="prefix"
                type="safety"
          ...
          <a-form-item label="Confirm Password">
            <a-input-password
              v-decorator="[
                ...
                {
                  rules: [
                    { required: true, message: 'Please confirm the reset password' },
                    { min: 8, message: 'Please enter 8~12 digit password' },
                    { max: 12, message: 'Please enter 8~12 digit password' },
                  ],
                },
              ...
              type="password"
              placeholder="Please confirm the reset password"
            >
              <a-icon
                slot="prefix"
                type="lock"
          ...
          <a-form-item :wrapper-col="{ span: 12, offset: mobile ? 0 : 6 }">
            ...
            <div>
              or 
              <a href="javascript:;" @click="loginFrom"> Return Login Pages </a>
            </div>
          </a-form-item>
        </a-form>
      </div>
  ...
  methods: {
    ...
    resetEmailCaptcha(e) {
      ...
      if (email == undefined) {
        ...
        // 60秒处理时间
        this.isCaptcha = true;
        this.$message.loading("Sending the code......", 0);
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          if (values.secondaryPassword == values.password) {
            ...
            this.$message.error("Two inconsistent passwords");
          }
        }
      });
    },
