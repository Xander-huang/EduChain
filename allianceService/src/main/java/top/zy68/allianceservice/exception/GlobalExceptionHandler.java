...
public class GlobalExceptionHandler {
    ...
    /**
     * Excel导入数据中，部分数据导入失败的异常，返回导入失败数据
     *
     * @param e FailedToImportException
     * @return ResponseBean
     */
    @ExceptionHandler(FailedToImportException.class)
    public ResponseBean handleClientBusinessException(FailedToImportException e) {
        return new ResponseBean(ResultCode.BAD_REQUEST, e.getData());
    }
     ...
     */
    @ExceptionHandler(UnauthorizedException.class)
    public ResponseBean handleRoleException() {
        return new ResponseBean(ResultCode.UNAUTHORIZED, "权限不足");
    }
    ...
    public ResponseBean globalException(Throwable ex) {
        return new ResponseBean(ResultCode.INTERNAL_SERVER_ERROR, "服务器错误.");
    }
}
